...
// Declarations
volatile bool isInfraRedFlag = true;
volatile bool isReadyToStartBarcode = false;
volatile int arrayIndex = 0;         
volatile int timingArray[9];       
volatile int timeDifferences[9]; 

// Character Logic Variables
volatile int startReadingIndex = 0;
int charTimingStartEndStar[9];    
volatile int starCount = 0;    
volatile int compareQValue = 0;     
char asciiCharacter = '~';       
char finalResultString[22];       
char sanitizedResultString[20];   
const int TIMEOUT_THRESHOLD = 1200; 

// Denoising
bool isTimingArrayNotMatched = true; 

// Backwards search
bool isBackwardsScanning = false;

// CODE39 Char Arrays. 1 is LONG bar. 0 is SHORT.
const int charStartEndStarPattern[9] = {0, 1, 0, 0, 1, 0, 1, 0, 0};
const int charSpacePattern[9] = {0, 1, 1, 0, 0, 0, 1, 0, 0};
...
int checkReadyToStartBarcode()
{
    ...
    {
        isReadyToStartBarcode = true;
        return 1; // For integration into the main loop
    };
}

void infraredScanning()
{
    if (gpio_get(GPIO_PIN_INFRARED_IN1) == 1) // BLACK BAR
    {
        if (isInfraRedFlag == true)
        {
            isInfraRedFlag = false;
            sleep_us(100);
            charTimingStartEndStar[arrayIndex] = time_us_32() / 1000;
            arrayIndex += 1;
        }
    }
    else
    {
        if (isInfraRedFlag == false)
        {
            isInfraRedFlag = true;
            sleep_us(100);
            charTimingStartEndStar[arrayIndex] = time_us_32() / 1000;
...
const char *returnCharacter()
{
    int tempCount = 0;
    int tempStarCount = 0;
    for (int i = 0; finalResultString[i] != '\0'; ++i)
    {
        if (finalResultString[i] != '*')
        {
            finalResultString[tempCount] = finalResultString[i];
            tempCount++;
        }
        if (finalResultString[i] == '*')
        {
...
void resetForNewString()
{
    starCount = 0;                    
    asciiCharacter = '~';               
    isTimingArrayNotMatched = true;         
    isBackwardsScanning = false;            
    strcpy(finalResultString, "");         
    strcpy(sanitizedResultString, "");    
    isReadyToStartBarcode = false;        
}
...
int main()
{
    ...
    // Main loop// MUST BE RUN FROM DEV CONSOLE: type Code and it will launch vsCode but you can build.
...
int infrared_main(){
    ...
    while (true)
    {
        ...
        if (INFRARED_readyToReturnChar())
        {
            ...
            printf("\n\n YOUR FINAL READING IS ON THE FOLLOWING LINE: \033[1;31m%s\033[0m",
                   final_char);

            strcpy(final_reading, final_char); // to send the reading to the wifi module
